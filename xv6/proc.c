...
scheduler(void)
{
  ...
  for(;;)
  {
	...
	continue;
      ...
      highP = p;
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
	  if((p->state == RUNNABLE) && (highP->priorityvalue > p->priorityvalue))
		highP = p; 
...
int waitS(int *status)
{
  ...
  for(;;){
	...
	p->state = UNUSED;
	release(&ptable.lock);
	*status = p->exit_status; // store exit_status to status
	return pid;
      }
    }
    ...
    if(!havekids || curproc->killed){
      release(&ptable.lock);
      *status = -1; // set status to -1 because no child exists
      return -1;
    }
